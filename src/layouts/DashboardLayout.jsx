...
export default function DashboardLayout({ role }) {
  ...
  return <div className="dashboard-shell">
    ...
    <div className="dashboard-body"><header className="dashboard-topbar"><div className="topbar-breadcrumb"><button className="icon-button mobile-only" aria-label="Open navigation" onClick={() => setOpen(true)}><Menu size={21} /></button><span className="role-label">{user?.role === 'superadmin' ? 'Super admin panel' : role + ' workspace'}</span><span className="breadcrumb-divider">/</span><strong>{items.find(([, , path]) => path === pathname)?.[0]}</strong></div><div className="user-profile"><span className="user-avatar">{user?.name?.charAt(0) || 'U'}</span><div><strong>{user?.name}</strong><span>{role === 'admin' ? (user?.role === 'superadmin' ? 'Global super administrator' : user?.institute_name || 'Institute admin') : role + ' account'}</span></div></div></header><main className="dashboard-content"><div className="page-enter" key={pathname + scope}><Outlet context={{ scope, changeScope, isSuperadmin: user?.role === 'superadmin' }} /></div></main></div>
  </div>;
}
