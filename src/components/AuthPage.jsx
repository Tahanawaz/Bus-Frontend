...
export default function AuthPage({ signup = false }) {
  ...
  const submit = async (e) => {
    ...
    try {
      if (signup) {
        ...
        navigate('/' + (['admin', 'driver'].includes(data.user.role) ? data.user.role : 'student'));
      }
    } catch (err) { toast.error(err.response?.data?.error || 'Unable to connect. Please try again.'); }
  ...
  return <div className="auth-page">
    ...
    <section className="auth-form-panel">
      ...
      </div><span className="auth-footnote">One campus. Every route. All connected.</span>
    </section>
  </div>;
}
