...
function LeafletViewport({ position }) {
  const map = useMap();
  useEffect(() => {
...
function OpenStreetMap({ buses, onSelect, position, selected, theme }) {
  return <MapContainer center={[position?.lat || DEFAULT_CENTER.lat, position?.lng || DEFAULT_CENTER.lng]} zoom={position ? 15 : 12} style={mapContainerStyle}>
    <LeafletViewport position={position}/>
    <TileLayer
    ...
    {buses.filter(bus => coordinates(bus)).map(bus => <Fragment key={bus.id}>
      <Marker position={coordinates(bus)} icon={leafletIcon} eventHandlers={{ click: () => onSelect?.(bus.id) }}>
        <Popup><div className="google-map-popup"><strong>{bus.name}</strong><span>{bus.number_plate}</span><p>{bus.status || 'No status reported'}</p><small>{bus.driver_name || 'Driver not assigned'}</small></div></Popup>
      </Marker>
    </Fragment>)}
  </MapContainer>;
}

function GoogleTrackingMap({ apiKey, buses, selectedId, onSelect, selected, position, theme }) {
  const [popupId, setPopupId] = useState(null);
  const { isLoaded, loadError } = useJsApiLoader({ id: 'smartbus-google-map', googleMapsApiKey: apiKey || '' });
  const center = position || DEFAULT_CENTER;
  const visibleBuses = buses.map(bus => ({ bus, position: toPoint(bus) })).filter(item => item.position);
  ...
  return <GoogleMap mapContainerStyle={mapContainerStyle} center={center} zoom={position ? 15 : 12} options={options}>
...
export default function TrackingMap({ buses, selectedId, onSelect }) {
  ...
  const mapBody = provider === 'google'
    ? <GoogleTrackingMap apiKey={apiKey} buses={buses} selectedId={selectedId} onSelect={onSelect} selected={selected} position={position} theme={theme}/>
    : <OpenStreetMap buses={buses} onSelect={onSelect} selected={selected} position={position} theme={theme}/>;

  return <section className="tracking-map-panel">
    <header><span className="mini-icon"><MapPin size={19}/></span><div><span className="eyebrow">LIVE MAP</span><h2>{selected?.name || 'Choose a bus to follow'}</h2><p>{selected ? (position ? 'Live location reported by the driver GPS' : 'Waiting for location from the driver') : 'Select a bus from the list below or beside the map.'}</p></div></header>
