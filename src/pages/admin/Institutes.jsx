...
export default function Institutes() {
  ...
  return <div className="management-page">
    ...
    <section className="management-panel"><div className="management-heading"><h2>All registered institutes</h2><DownloadPdfButton type="institutes" params={{ institute_id: 'all' }} /></div><div className="institute-cards" role="region" aria-label="Registered institutes" tabIndex={0}>{institutes.map(i=><article key={i.id}><div><Building2 size={22}/><button disabled={saving} className="icon-button" aria-label={'Edit '+i.name} onClick={()=>{setEditing(i.id);setInstitute({name:i.name,address:i.address});setInstituteOpen(true);}}><Pencil size={17}/></button><button type="button" disabled={saving} className="icon-button" aria-label={'Delete '+i.name} onClick={()=>removeInstitute(i)}><Trash2 size={17}/></button></div><h3>{i.name}</h3><p>{i.address||'Address not added'}</p><span>{admins.find(a=>a.institute_id===i.id)?.name || 'No admin assigned'}</span><button type="button" className="secondary-button institute-admin-action" disabled={saving} onClick={()=>{const current=admins.find(a=>a.institute_id===i.id);setReplacing(!!current);setAdminEditing(current?.id||null);setAdmin({name:'',email:'',password:'',institute_id:i.id});setAdminOpen(true);}}>{admins.some(a=>a.institute_id===i.id)?'Change admin':'Assign admin'}</button></article>)}</div></section>
