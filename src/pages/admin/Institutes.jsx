...
export default function Institutes() {
  ...
  const saveAdmin = async e => {
    e.preventDefault(); setSaving(true);
    try { if(adminEditing) await api.put('/auth/admins/'+adminEditing,admin); else await api.post('/auth/admins',admin); setAdminOpen(false); setAdminEditing(null); setAdmin({ name:'',email:'',password:'',institute_id:'' }); await load(); toast.success('Institute administrator saved.'); }
  ...
  return <div className="management-page">
    <header className="management-heading"><div><span className="eyebrow">HAMARA PANEL</span><h1>Institutes & administrators</h1><p>Create separate workspaces for every campus. You retain access to all institutes.</p></div><div className="form-actions"><button className="primary-button" disabled={saving} onClick={()=>{setEditing(null);setInstitute({name:'',address:''});setInstituteOpen(true);}}><Plus size={17}/>Add institute</button><button className="primary-button" disabled={saving} onClick={()=>{setAdminEditing(null);setAdmin({name:'',email:'',password:'',institute_id:''});setAdminOpen(true);}}><Plus size={17}/>Add admin</button></div></header>
    <section className="management-panel"><div className="management-heading"><h2>Your institutes</h2><DownloadPdfButton type="institutes" params={{ institute_id: 'all' }} /></div><div className="institute-cards">{institutes.map(i=><article key={i.id}><div><Building2 size={22}/><button disabled={saving} className="icon-button" aria-label={'Edit '+i.name} onClick={()=>{setEditing(i.id);setInstitute({name:i.name,address:i.address});setInstituteOpen(true);}}><Pencil size={17}/></button></div><h3>{i.name}</h3><p>{i.address||'Address not added'}</p><span>{admins.filter(a=>a.institute_id===i.id).length} administrator(s)</span></article>)}</div></section>
    <section className="management-panel"><div className="management-heading"><h2>Institute administrators</h2><DownloadPdfButton type="admins" params={{ institute_id: 'all' }} /></div><div className="management-table-wrap"><table><thead><tr><th>Name</th><th>Email</th><th>Institute</th><th>Actions</th></tr></thead><tbody>{admins.map(a=><tr key={a.id}><td>{a.name}</td><td>{a.email}</td><td>{a.institute_name}</td><td><div className="row-actions"><button disabled={saving} aria-label={'Edit '+a.name} onClick={()=>{setAdminEditing(a.id);setAdmin({name:a.name,email:a.email,password:'',institute_id:a.institute_id});setAdminOpen(true);}}><Pencil size={16}/></button><button disabled={saving} aria-label={'Remove '+a.name} onClick={()=>removeAdmin(a)}><Trash2 size={16}/></button></div></td></tr>)}</tbody></table>{!admins.length&&<p className="empty-state">Create an administrator for an institute to get started.</p>}</div></section>
    ...
    <Dialog open={adminOpen} onClose={()=>!saving&&setAdminOpen(false)} fullWidth maxWidth="sm" aria-labelledby="admin-dialog-title">
      <DialogTitle id="admin-dialog-title">{adminEditing?'Edit admin':'Add admin'}</DialogTitle>
      <DialogContent><form className="management-form dialog-form" onSubmit={saveAdmin}>
        ...
        <label>Institute<select autoFocus required disabled={!!adminEditing} value={admin.institute_id} onChange={e=>setAdmin({...admin,institute_id:e.target.value})}><option value="">Select institute</option>{institutes.map(i=><option key={i.id} value={i.id}>{i.name}</option>)}</select></label>
        ...
        <label>{adminEditing?'New password (leave blank to keep)':'Initial password'}<input type="password" required={!adminEditing} minLength={6} autoComplete="new-password" value={admin.password} onChange={e=>setAdmin({...admin,password:e.target.value})}/></label>
